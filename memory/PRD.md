...
- 2026-06 : **Formule "réels ou prévus"** : `temps_formateur = durée × max(nb_formateurs_réels, nb_formateurs_requis)` partout (`/recap-ue` et `/workload`). Les séances sans formateur encore assigné mais avec un `nb_formateurs_requis` défini sont désormais comptées dans le volume global à pourvoir, pour ne pas sous-estimer la charge restante. La répartition par formateur reste basée sur les assignations réelles. Cohérence vérifiée 71.5h ↔ 71.5h.
