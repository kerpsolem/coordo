...
- 2026-06 : **Cohérence calculs heures** : "Volume global cours à pourvoir" (Par formateur) et "Heures cours assignées" (Récap) sont **la même métrique** (`total_cours_assignees` = somme durée × formateurs réels). Référence pour le théorique du formateur = `total_cours_assignees × quotité_i / capacité_totale`. La somme des heures cours par formateur égale exactement le total assignées (vérifié). "Heures à pourvoir" calculé par séance (`Σ max(0, nb_req − have) × duree`) au lieu de la différence globale (qui se compensait avec les surnombres → affichait 0h à tort).
