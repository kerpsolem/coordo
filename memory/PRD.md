...
- 2026-06 : **Cohérence calculs heures (v2)** : `/workload` aligné sur `/recap-ue.total_temps_formateur`. Formule unique : `Σ (duree × nb_formateurs × nb_groupes)` pour tous les types sauf TPG, sur sessions + activités de fiches projet non programmées (UE non nulle requise). Référence par formateur = `total_temps_formateur × quotité_i / capacité_totale`. Par construction, `Σ heures_cours_par_formateur = total_temps_formateur`. Heures à pourvoir par séance multipliées par `nb_groupes`. Filtres semestre/promotion appliqués aussi sur les fiches projet (vérifié : S1=133h, S3=38h, total=231h identique dans les deux endpoints).
