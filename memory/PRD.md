...
- 2026-06 : **Cohérence calculs heures (v3)** : suppression de l'inclusion des activités de fiches projet non programmées dans `/workload` et `/recap-ue` (risque de doublons quand une activité est recréée dans plusieurs fiches). Les calculs (Volume global, Heures à pourvoir, Théorique, Écart) se basent uniquement sur les sessions effectivement programmées. Cohérence vérifiée /workload ↔ /recap-ue sur 4 filtres (99h annuel, 2h juin, 67h S1).
