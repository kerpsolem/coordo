...
- 2026-06 : **Par promo : édition rapide saisi + salles** : sur la vue Par promotion, les profils super_admin / admin_coordination / secretariat peuvent désormais cliquer sur une activité (carte du planning ou ligne du tableau) pour ouvrir une modale d'édition rapide permettant de toggler `saisi` (Oui/Non) et de sélectionner les salles (multi-select). Backend `/api/sessions/{id}` PUT élargi : secretariat autorisé mais limité aux champs `saisi` et `site_ids`.
